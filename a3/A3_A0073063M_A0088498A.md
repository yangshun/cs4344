...
Of course we need a way for other clients to know when the rocket is destroyed. In order to achieve this, clients will simulate collisions locally and discard any rockets that have collided with ships.
...
Although effective, this implementation has two flaws. Firstly, the client now also needs to simulate part of the game. Secondly, this implementation is not guaranteed to work in a high latency environment. However, since this consistency is not a focus of this assignment, this approach is acceptable.
...
We wanted to compare the effects of our interest management implementation, hence we logged the server traffic while the game was going on for 5 mins, with 50 bots in the game. We created a flag in `Config.js` called `INTEREST_MANAGEMENT`, which can be used to toggle whether interest management was activated in the server.
